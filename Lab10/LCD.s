...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	  PUSH {R4-R11}
busy  LDR R3, =SSI0_SR_R
	  LDR R1, [R3]
	  AND R1, R1, #0x10
	  LSR R1, #4 ;1) Read SSI0_SR_R and check bit 4, 
	  CMP R1, #1
	  BEQ busy ;2) If bit 4 is high, loop back to step 1 (wait for BUSY bit to be low)
	  LDR R3, =DC
	  LDR R2, [R3]
	  BIC R2, R2, #0x40 ;3) Clear D/C=PA6 to zero
	  STRB R2, [R3]
	  LDR R3, =SSI0_DR_R 
	  STRB R0, [R3] ;4 Write the command to SSI0_DR_R
step  LDR R3, =SSI0_SR_R
	  LDR R1, [R3]
	  AND R1, R1, #0x10
	  LSR R1, #4 ;5) Read SSI0_SR_R and check bit 4, 
	  CMP R1, #1
	  BEQ step ;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
	  POP {R4-R11}
      BX  LR                          ;   return
...
;4) Write the 8-bit data to SSI0_DR_R
	 PUSH {R4-R11}
loop LDR R3, =SSI0_SR_R
	 LDR R1, [R3]
	 AND R1, R1, #0x02
	 LSR R1, #1 ;1) Read SSI0_SR_R and check bit 1, 
	 CMP R1, #0x00 
	 BEQ loop ;2) If bit 1 is low loop back to step 1 (wait for TNF bit to be high)
	 LDR R3, =DC
	 LDR R2, [R3]
	 BIC R2, R2, #0x40
	 ORR R2, R2, #0x40 ;3) Set D/C=PA6 to one
	 STRB R2, [R3]
	 LDR R3, =SSI0_DR_R
	 STRB R0, [R3] ;4) Write the 8-bit data to SSI0_DR_R
	 POP {R4-R11}
     BX  LR                          ;   return
